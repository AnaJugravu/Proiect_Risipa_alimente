...
export const create = async (req, res) => {
    if (!req.body.denumire || !req.body.personId) {
        return res.status(404).send({ message: "Missing name or associated person" });
    }

    const existingCollection = await collectionsService.getCollections({ denumire: req.body.denumire, personId: req.body.personId });
